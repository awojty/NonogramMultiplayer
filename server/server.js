...
function newGame(room, width, height){
    currentRoom = rooms.get(room)
    const board = new Board(width, height)
    console.log("board in newGame", board)
...
io.on('connection', socket =>{
    ...
    socket.on('joining', ({room}) => {
        console.log("try to joing this", room)
        console.log("joiing rooms");
    ...
    socket.on('newRoomJoin', ({room,name, width, height})=>{
        ...
        if (peopleInRoom>=1){
            ...
            console.log("starting gameboard",currentRoom.board.game )

            
            const players = currentRoom.players.map((player) => [player.id, player.name])
            io.to(room).emit('starting', {gameState})
        } 
